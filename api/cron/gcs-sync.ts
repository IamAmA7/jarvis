...
export default async function handler(req: any): Promise<Response> {
  try {
    ...
    console.error('[gcs-sync] start: cron header=', getHeader(req, 'x-vercel-cron'));
    verifyCron(req);
    console.error('[gcs-sync] verified');
    ...
    console.error('[gcs-sync] bucket=', bucket, 'prefix=', prefix);

    console.error('[gcs-sync] getting access token...');
    const token = await getAccessToken();
    console.error('[gcs-sync] got token (len=', token.length, ')');

    console.error('[gcs-sync] listing objects...');
    const objects = await listObjects(bucket, token, prefix);
    console.error('[gcs-sync] listed', objects.length, 'objects');

    console.error('[gcs-sync] loading seen from supabase...');
    const seen = await loadSeen(bucket);
    console.error('[gcs-sync] loaded', seen.size, 'seen entries');
    const fresh = objects
